...
{	// redéclaration des données globales utilisées dans la fonction
	...
	$dao = new DAO();
	$dao->annulerReservation($numRes);
	
	$message = "La réservation à bien été annulé";
	$user = $dao->getUtilisateur($nom);
	$email = $user->getEmail();
	Outils::envoyerMail ($email, "Confirmation annulation", $message, "delasalle.sio.crib@gmail.com");
	return;
}
?>
